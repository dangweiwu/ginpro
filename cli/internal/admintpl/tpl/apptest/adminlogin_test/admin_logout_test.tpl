...
func logout(t *testing.T) {
	...
	if assert.Equal(t, w.Code, 200, w.Body.String()) {
		...
		id, err := sc.Redis.Get(context.Background(), adminmodel.GetAdminRedisRefreshTokenId(int(po.ID))).Result()

		assert.Equal(t, len(id), 0, id)
		assert.Equal(t, err, redis.Nil)

		id, err = sc.Redis.Get(context.Background(), adminmodel.GetAdminRedisLoginId(int(po.ID))).Result()
		assert.Equal(t, len(id), 0, id)
		assert.Equal(t, err, redis.Nil)
